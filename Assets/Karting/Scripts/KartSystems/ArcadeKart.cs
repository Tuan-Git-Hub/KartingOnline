...
namespace KartGame.KartSystems
{
    public class ArcadeKart : NetworkBehaviour
    {
        ...
        private InputData lastReceivedInput;
        private List<(InputData, int)> inputList = new List<(InputData, int)> ();
        private int seqID = 0;
        ...
        public override void OnStartLocalPlayer()
        {
            ...
            var hud = FindFirstObjectByType<HUDInputMobile>();
            if (hud != null)
            {
                hud.BindPlayer(this);
            }
        ...
        private void Update()
        {
            if (isLocalPlayer)
            {
                CmdSendTransformServer(transform.position, transform.rotation, seqID);
            }
        }

        [Command]
        void CmdSendInputToServer(InputData inputData)
        {
            //lastSeqID = seqID;
            RpcBroadcastInput(inputData);
        }

        [Command]
        void CmdSendTransformServer(Vector3 position, Quaternion rotation, int seqID)
        {
            RpcSyncState(position, rotation, seqID);
        }

        [ClientRpc]
        void RpcBroadcastInput(InputData inputData)
        {
            if (!isLocalPlayer)
            {
                lastReceivedInput = inputData;
            }
        }

        [ClientRpc]
        void RpcSyncState(Vector3 pos, Quaternion rot, int confirmedSeqID)
        {
            if (!isLocalPlayer)
            { 
            ...
            inputList.RemoveAll(i => i.Item2 <= confirmedSeqID);
        ...
        void GatherInputs()
        {
            if (GameManager.Instance.haveCountdownTimeBeforeRace) return;
            // reset input
            Input = new InputData();
            WantsToDrift = false;
            ...
            if (isLocalPlayer)
            {
                //Debug.Log("m_Inputs.Length: " + m_Inputs.Length);
                // gather nonzero input from our sources
                for (int i = 0; i < m_Inputs.Length; i++)
                {
                    var inP = m_Inputs[i].GenerateInput();
                    inP.Accelerate |= Input.Accelerate;
                    inP.Brake |= Input.Brake;
                    inP.TurnInput += Input.TurnInput;
                    Input = inP;
                    //Debug.Log($"Inputs{i}: {Input.Accelerate}, {Input.Brake}, {Input.TurnInput}");
                }
